  ...
  async recordTelemetry(input: RecordTelemetryInput) {
    return withDb(async () => {
      const zoneRows = await db
        .select()
        .from(dropZones)
        .where(eq(dropZones.id, input.zoneId))
        .limit(1);
      ...
      const [updatedZone] = await db
        .update(dropZones)
        .set({
          current_fill_kg: currentFill.toFixed(2),
          ...(isEmptied ? { last_emptied_at: new Date() } : {}),
        })
        .where(eq(dropZones.id, input.zoneId))
        .returning();
  ...
  async generatePoster(zoneId: string, options: PosterOptions = {}) {
    return withDb(async () => {
      ...
      if (!qrToken || !isValidQrToken(qrToken)) {
        ...
        await db.update(dropZones).set({ qr_token: qrToken }).where(eq(dropZones.id, zoneId));
      }
...
<body>
  ...
  async getAdminTelemetryOverview() {
    return withDb(async () => {
      const allZones = await db.select().from(dropZones).orderBy(desc(dropZones.created_at));
